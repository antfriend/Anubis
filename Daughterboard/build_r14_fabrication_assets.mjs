...
const outDir = path.join(root, "fabrication", "jlcpcb_2026-07-10_r14");
...
function normalizeValue(ref, value) {
  if (ref === "Q1") return "CSD18502Q5B charge FET";
  if (ref === "Q2") return "CSD18502Q5B discharge FET";
...
function overrideAudit(ref, value, footprint, audit) {
  if (ref === "Q1" || ref === "Q2") {
    return {
      ...
      MPN: "CSD18502Q5B",
      JLCType: "Extended",
      Note: "R14 high-side protection FET; TI DNK/VSON-CLIP pinout matched in schematic and PCB footprint.",
    };
  }
  if (ref === "R49" || ref === "R50") {
    return {
      ...
      JLCType: "",
      Note: "R14 gate-source bleed resistor; select a JLC 0603 10M part before assembly upload.",
    };
  }
  if (ref === "C32" || ref === "C33") {
    return {
  ...
  return audit || {
    ...
    MPN: "",
    JLCType: "",
    Note: "No carried-forward JLC part match found for this R14 item.",
  };
}
...
await fs.writeFile(
  path.join(outDir, "Daughterboard_r14_jlc_part_audit.csv"),
  toCsv(["Status", "Designator", "Comment", "Footprint", "Quantity", "LCSC", "MPN", "JLCType", "Note"], auditRows),
);
await fs.writeFile(
  path.join(reportsDir, "Daughterboard_r14_missing_jlc_parts.csv"),
