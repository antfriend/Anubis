...
Current routed fabrication package:
...
- R14 swaps Q1/Q2 from `CSD16412Q5A` to JLC-available TI `CSD18502Q5B`, JLC/LCSC `C473915`.
- The schematic now uses a real 1-8 pin `CSD18502Q5B` symbol: pins 1-3 source, pin 4 gate, pins 5-8 drain.
- The PCB now uses project-local footprint `Daughterboard:CSD18502Q5B_DNK0008A_VSON-CLIP_5x6mm` for Q1/Q2.
- Routed traces/vias were intentionally cleared for the Q1/Q2 footprint swap, then the board was rerouted for R14.
- Zones were refilled after the R14 reroute.
- R14 ERC is clean: 0 violations in `r14_post_route_erc.rpt`.
- R14 topology audit passes in `verify_r14_power_topology.py`.
- R14 DRC currently reports 40 non-connectivity violations and 0 unconnected pads/items in `r14_post_route_drc.rpt`.
- Backup before the R14 swap is `Daughterboard.pre_r14_csd18502_swap.kicad_pcb` and `Daughterboard.pre_r14_csd18502_swap.kicad_sch`.
...
- R13 rewired the BQ28Z610 protection topology from the old incompatible low-side FET path to a high-side N-FET path. R14 keeps that topology and uses two `CSD18502Q5B` FETs: `Q1` charge FET and `Q2` discharge FET.
...
- In the active R14 working files, Q1/Q2 are selected as `CSD18502Q5B`, JLC/LCSC `C473915`.
- JLC assembly still needs confirmed selections for `R49` and `R50`, the 10M 0603 gate-source bleed resistors.
...
- R13 replaced the old `CSD83325L`/`PT8810` low-side FET position with two high-side protection FETs. R14 changed those FETs to `CSD18502Q5B` so JLCPCB can assemble Q1/Q2 using an in-stock part.
...
3. Confirm JLC orientation before ordering:
   - Especially U1/BQ25887, U2/BQ28Z610, Q1/Q2/CSD18502Q5B, USB-C, JST connectors, and ESD arrays.
