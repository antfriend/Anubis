...
flowchart LR
    ...
    CHG <--> PROT["BQ28Z610 + 2x CSD16412Q5A\nhigh-side 2S protection"]
...
| 2S gauge/protection | TI `BQ28Z610DRZR-R1` + 2x TI `CSD16412Q5A` | 1S/2S Impedance Track gauge/protection controller with high-side N-FET charge/discharge control and current sense. Requires BQStudio/data-flash setup. |
...
- `Q1`: `CSD16412Q5A` high-side charge FET, drain on `BAT_SYS`, source on `FET_SRC_COMMON`, gate on `CO_GATE`.
- `Q2`: `CSD16412Q5A` high-side discharge FET, drain on `FET_BATT_P`, source on `FET_SRC_COMMON`, gate on `DO_GATE`.
...
R13 release note: the previous low-side dual-FET topology was removed because it was incompatible with the BQ28Z610 high-side gate driver. The charger `BAT/SNS` pins and the buck-regulator input bus are now on `BAT_SYS`, so charge and discharge current pass through the high-side protection FETs and through `R46`.
...
- TI CSD16412Q5A product/datasheet: https://www.ti.com/product/CSD16412Q5A
